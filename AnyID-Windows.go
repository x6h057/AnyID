...
func main() {
	tmpFile := "reset_anydesk.bat"

	err := os.WriteFile(tmpFile, []byte(batScript), 0644)
	if err != nil {
		fmt.Println("[!] Failed to write batch file:", err)
		return
	}

	fmt.Println("[*] Executing embedded batch script...")
	cmd := exec.Command("cmd", "/C", tmpFile)
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	cmd.Stdin = os.Stdin // So pause works
	...
	fmt.Println("[✓] Batch script completed.")
}
